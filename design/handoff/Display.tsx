...
// KaTeX: `import "katex/dist/katex.min.css"` once in the app; render with
...
export interface RpnState {
  T: number; Z: number; Y: number; X: number;
  lastX: number;
...
function StackRow({ label, value, muted, fmt: _fmt }: { label: string; value: string; muted?: boolean; fmt?: unknown }) {
  return (
    <div className="flex items-baseline justify-between py-px">
      <span className="font-mono text-[11px] font-semibold tracking-[0.1em] text-hp-display-dim">{label}</span>
      <span className={cn("text-[17px]", muted ? "font-display text-hp-display-dim" : num)}>{value}</span>
    </div>
  );
}

export function Display({ state: s, family, showAngle, showRegisters, renderLatex, fmt }: DisplayProps) {
  ...
  return (
    <div className="flex flex-col rounded-lg border border-hp-display-border bg-hp-display p-4 shadow-[inset_0_1px_0_rgb(255_255_255/0.22),inset_0_-10px_20px_rgb(0_0_0/0.14)]">
      ...
      {/* KaTeX hero */}
      <div className="flex min-h-[52px] items-center text-[30px] text-hp-display-fg" dangerouslySetInnerHTML={renderLatex(s.latex)} />
      ...
      {isRpl ? (
        ...
        <div className="mt-2">
          ...
          <div className="flex items-baseline justify-between">
            <span className="font-mono text-[11px] font-bold tracking-[0.1em] text-hp-display-fg opacity-70">X</span>
            <span className={cn(num, "text-[30px]")}>{s.entry != null ? s.entry : fmt(s.X, s.dec)}</span>
          </div>
      ...
      {showRegisters && s.reg && (
        <div className="mt-2.5 flex flex-wrap gap-2.5 border-t border-hp-display-dim pt-2">
          {(["n", "i", "PV", "PMT", "FV"] as const).map((k) => (
            <span key={k} className={cn("font-mono text-[10px]", s.reg![k] ? "text-hp-display-fg" : "text-hp-display-dim")}>
...
function RplStack({ rpl, entry, fmt }: { rpl: number[]; entry: string | null; fmt: (n: number) => string }) {
  const start = Math.max(0, rpl.length - 6);
  const lines = rpl.slice(start).map((val, i) => ({ lvl: rpl.length - (start + i), val: fmt(val) }));
  return (
    <div className="mt-1 flex flex-1 flex-col justify-end gap-px">
      {lines.length === 0 ? (
        <div className="py-5 text-center font-mono text-[13px] text-hp-display-dim">( empty stack )</div>
      ) : (
        lines.map((l) => (
          <div key={l.lvl} className="flex items-baseline gap-2.5 border-t border-black/10 py-[3px]">
            <span className="min-w-[20px] font-mono text-[11px] text-hp-display-dim">{l.lvl}:</span>
...
export function StackPanel({ state: s, family, fmt }: StackPanelProps) {
  ...
  const rows = isRpl
    ? (() => {
        ...
        for (let i = st.length - 1; i >= Math.max(0, st.length - 5); i--) out.push({ lab: `${st.length - i}:`, val: fmt(st[i], s.dec), hot: i === st.length - 1 });
        return out.length ? out : [{ lab: "1:", val: "—", hot: true }];
      })()
    : (["T", "Z", "Y", "X"] as const).map((k) => ({ lab: k, val: fmt(s[k], s.dec), hot: k === "X" }));
  ...
  return (
    <aside className="flex max-w-[420px] min-w-[280px] flex-1 flex-col self-stretch rounded-[var(--radius-bezel)] border border-border bg-hp-panel p-5">
      <h2 className="mb-3.5 font-mono text-[10.5px] font-semibold tracking-[0.18em] text-muted-foreground uppercase">History</h2>
      <div className="flex min-h-[200px] flex-1 flex-col gap-2.5 overflow-y-auto">
        {(s.hist ?? []).slice().reverse().map((e, i) => (
          <div key={i} className="flex items-baseline justify-between border-b border-border pb-2">
            <span className="font-legend text-[13px] font-semibold text-muted-foreground">{e.op}</span>
            <span className="font-mono text-[14px] text-foreground">{e.v}</span>
          </div>
        ))}
      </div>
      <div className="mt-3.5 border-t border-border pt-3.5">
        <h3 className="mb-2.5 font-mono text-[10.5px] font-semibold tracking-[0.18em] text-muted-foreground uppercase">{isRpl ? "RPL Stack" : "RPN Stack"}</h3>
        {rows.map((r, i) => (
          <div key={i} className="flex justify-between py-0.5">
            <span className="font-mono text-[12px] tracking-[0.1em] text-muted-foreground">{r.lab}</span>
            <span className={cn("font-mono text-[14px]", r.hot ? "text-primary" : "text-foreground")}>{r.val}</span>
          </div>
        ))}
      </div>
    </aside>
