...
## Goal
Deliver the HP-35s — the 2007 retro-styled RPN scientific — by adding the last engine pieces a
...
constants library** (`CONST` menu), 2-D/3-D **vector** arithmetic, and the two-line dot-matrix
display with its yellow/blue shift scheme. Everything reuses the P1 BigNumber tower, the P9
root-finder/integrator, and the P16 stats registers — this phase is mostly integration plus the
constants dataset and the RPN/ALG entry switch.
...
- **HP-35s** (2007) — yellow left-shift (←) / blue right-shift (→) Voyager-grid scientific with a
  ...
  A–Z + indirect I/J. Faceplate per `hp/layouts/HP-35s.md`, functions per `hp/functions/HP-35s.md`.
...
- **Fraction display + IDIV/RMDR/IP/FP/SGN** equation functions and the base/logic menu reuse from
  P10 (`DEC`/`HEX`/`OCT`/`BIN`, `AND`/`OR`/`XOR`/`NOT`/`NAND`/`NOR`).
...
- **FR-MODEL-1/2/3/5** — faithful two-line dot-matrix faceplate; key + ←/→ prefix dispatch via
  `mapping.json`; only HP-35s functions exposed; ALG/RPN/EQN/annunciator row.
...
- **Faceplate / UI:** HP-35s faceplate from `hp/layouts/HP-35s.md` (three functions/key + A..Z
  alpha); two-line display (entry line + result line); annunciator row (ALG/RPN/EQN/GRAD/A..Z/
  HYP/HEX/OCT/BIN); cursor-navigated menus (no softkeys); CONST picker.
...
- e2e: MODE switches RPN⇄ALG and both compute `5` from the same keys' meaning; `EQN … SOLVE`
  prompts for the unknown and returns the root; CONST menu pushes a constant; a 3-D vector entry
  renders in the display.
- `pnpm lint` / `pnpm test` / `pnpm build` / `pnpm test:e2e` green; fidelity vs `hp/layouts/HP-35s.md`.
