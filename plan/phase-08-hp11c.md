...
**Delivers:** HP-11C · probability (nPr / nCr / x! / RAND) on the Voyager scientific-programmable faceplate · **Era:** 1981 · **Builds on:** Phase 6 (scientific fns + keystroke programming; reuses Phase 2 stats/conversions and Phase 5 indirect addressing)

## Goal
Deliver the **HP-11C** — the scientific-programmable member of the Voyager line — as the first
Voyager-styled scientific faceplate. Almost all of its engine capability already exists from
earlier phases (scientific functions, statistics, linear regression, keystroke programming,
I-register indirection). The one genuinely new engine addition is **combinatorics /
probability**: permutations, combinations, factorial/gamma, and a pseudo-random generator.

## Models delivered
- **HP-11C** (1981) — 10-digit Voyager LCD, f (gold) / g (blue) prefixes, 4-level RPN,
  keystroke programming, hyperbolics, linear regression, probability. Faceplate per
  `hp/layouts/HP-11C.md`, functions per `hp/functions/HP-11C.md`.
...
- **Combinatorics / probability (new):**
  ...
  - `x!` — factorial for non-negative integers, generalized to Γ(1+x) for non-integers.
  - `RAN#` — seedable pseudo-random number in [0,1) (deterministic seed for testable output).
- **Reused (no new work, just exposed on this faceplate):** hyperbolic + inverse-hyperbolic
...
- **Faceplate / UI:** Voyager 4-row keyboard with f/g legends/colors, gold A–E user-label keys,
  USER annunciator; reuse the Voyager display shell (shared with the 12C from Phase 7).
- **Tests:** engine unit tests for probability + Playwright e2e on the faceplate.
...
- Faceplate e2e: `2.5 f HYP SIN` (hyperbolic sine) and an `L.R.` regression flow render the
  expected results; USER mode remaps A–E to stored labels.
- Fidelity: keys/colors/prefixes match `hp/layouts/HP-11C.md` (SM-1).
...
- The Voyager display/keyboard shell is shared with the HP-12C (Phase 7) and the later 15C/16C;
  factor the common Voyager faceplate framework here if not already extracted.
